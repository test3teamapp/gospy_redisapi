...
router.get('/byName/:name/command/:command', async (req, httpResponse) => {
    ...
    if (person == null) {
        ...
        if (numOfTracks == 0 && command != "TRIGGER_LU") {
...
// for calss from spring version of GoSpy

var springFcmCallbackLog = function (name, keyName, command, httpResponse, err, res) {
    //console.log("\n__________________________________")
    //console.log("\t" + command + " to " + name);
    //console.log("----------------------------------")
    //console.log("err=" + err);
...
router.get('/spring/byName/:name/command/:command', async (req, httpResponse) => {
    const name = req.params.name
    const command = req.params.command
    ...
    if (person == null) {
        ...
        if (numOfTracks == 0 && command != "TRIGGER_LU") {
            ...
            fcm.send(payload, function (err, res) {
                springFcmCallbackLog(name, keyName, command, httpResponse, err, res);
            });
        }
    }
});
